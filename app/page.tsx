...
export default function GamePage() {
    ...
    useEffect(() => {
        ...
        const resize = () => {
            ...
            // Sync isMobile state with breakpoint (1024px for lg)
    ...
    return (
        <div ref={containerRef} className="relative w-full h-[100dvh] bg-slate-950 overflow-hidden touch-none">
            ...
            {/* Desktop Vertical Ad Unit (Only rendered on desktop) */}
            {!isMobile && (
                <div className="fixed top-0 bottom-0 left-[240px] z-10 flex flex-col w-[160px] pointer-events-auto bg-black/20 border-r border-white/5 overflow-hidden backdrop-blur-sm">
                    ...
                    <div className="flex-1 flex items-center justify-center p-2 text-white/5">
                        <AdUnit adSlot="7585529595" adFormat="auto" />
                    </div>
                </div>
            )}

            {/* Mobile Horizontal Ad Unit (Only rendered on mobile) */}
            {isMobile && (
                <div className={`fixed bottom-0 left-0 right-0 z-10 flex flex-col bg-black/40 border-t border-white/10 transition-transform duration-300 ${state.selection ? 'translate-y-[-140px]' : 'translate-y-0'}`}>
